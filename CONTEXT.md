...
- **Secret Vault**: the only Module allowed to persist structured credential fields. It stores opaque `secret://` references in SQLite and resolves them at runtime through a system keychain Adapter or an encrypted-file Adapter.
...
1. Persisted structured secret fields (Auth secret parameters, `type=secret` variables, and the sync password) are opaque `secret://keyring/` or `secret://file/` references. Arbitrary strings in URLs, ordinary headers, bodies, and scripts remain request content and are not automatically classified as credentials.
