...
GetHistoryBody(historyId string) (model.ResponseBody, error) // 大 body 按需拉取
...
其中 `SendRequest` / `ListNodes` / `UpsertNode` / `ListHistory` 为 [Phase 1 任务分解](./roadmap.md#4-phase-1-任务分解可直接开工)的最小必做集，其余随 Phase 1 UI 需要落地。
...
// ExampleApi（Phase 3/4，响应"保存为示例" + Mock 数据源）
...
// ProtocolApi（Phase 4，WS/SSE/gRPC 会话）
