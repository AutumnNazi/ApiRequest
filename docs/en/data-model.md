...
-- Runner run history (ADR-015 reverses the earlier in-memory decision; summary columns back the
-- Retention: only the most recent 200 runs per (workspace_id, collection_id) are kept; older rows are pruned automatically
-- (the cap is adjustable in Settings -> Storage -> Retention, setting key retention.runnerRuns; history likewise via retention.history, default 1000)
-- list projection, the detail JSON column is read only when a single run is opened).
CREATE TABLE runner_run (
  id            TEXT PRIMARY KEY,                   -- = runId (generated by the frontend)
  ...
  results       TEXT NOT NULL DEFAULT '[]'          -- JSON: RequestResult[]
);
...
- **Runner reports**: ~~memory only~~ **reversed (2026-09-04, [ADR-015](./decisions.md#adr-015-persist-runner-run-reports-to-the-database-accepted-2026-09-04-reverses-the-earlier-in-memory-decision))**: reports are written to the `runner_run` table when a run finishes (summary columns plus a detail JSON column, schema 0010); the Runner dialog can review past runs, delete one, or clear all. A persistence failure never blocks returning the report. The most recent 200 runs per (workspace, collection) are kept automatically (the cap is adjustable in Settings -> Storage) so detail JSON cannot grow without bound.
