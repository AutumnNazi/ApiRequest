...
| Import-parser fuzzing | all six import formats (cURL/HAR/Insomnia/OpenAPI/Postman/REST Client) never panic on arbitrary input | CI runs a 60s probe per push plus seed-corpus regression (`go test -fuzz`) |
...
| Installer size | < 30 MB per platform; prefer CM6 to Monaco, with Go + system WebView far smaller than Electron; CI enforces this before artifact upload and release (`scripts/check-dist-budget.mjs`), failing the build when exceeded |
