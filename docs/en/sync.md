...
- ~~Remote workspace discovery/import~~ now supported: the sync section in Settings lists workspace snapshots under the remote `ApiRequest/` directory (PROPFIND) and imports one with a single click — the local workspace is created with the remote's workspace ID, entities are written locally, and a merge baseline is stored, so regular two-way sync works immediately after import. Note: the remote path is still derived from the workspace ID; users already syncing across devices must not change local workspace IDs casually.
