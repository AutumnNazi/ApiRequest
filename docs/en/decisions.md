...
### ADR-018 Signed Update Protocol: Manifest + ed25519 + Channel Files + Two-Stage Atomic Replacement (Implemented; Release-Side Keygen Deferred Indefinitely 2026-09-09)

- **Status**: the verification chain and replace/rollback code are implemented and off by default (see OPEN-006). **The release-side key (`updatetool keygen` + pinning the public key + the manifest publishing workflow) is deferred indefinitely**: the audience is developers who face no friction updating via manual download or winget, so the silent updater's target population (non-technical users who linger on old versions) barely exists; with an empty public key verification fails closed (manifests are rejected outright), so there is no unsigned-update downgrade path and deferral carries zero security cost; no manifest has ever been published, so deferring creates no key-migration burden. Configure it whenever a real silent-update need appears (keygen and the signed-publishing code are ready).
...
- **Interim decision (2026-09-06)**: the signature verification chain and replacement/rollback are both implemented (`backend/updater`: channel manifest + ed25519 verification + version floor + sha256 download verification + two-stage replacement/rollback; the `cmd/updatetool` signing tool). Disabled by default — "Check for updates / Download & install" in Settings only works once `update.manifestUrl` is configured. Unix swaps the binary in place (effective on restart); Windows writes a `pending-update` marker and the startup hook verifies → backs up → swaps, rolling back and clearing the marker on failure.
- **Release side deferred (2026-09-09)**: no keypair is generated and no manifest is published; users update via "Check for updates → open the release page" or a package manager. Rationale in [ADR-018](#adr-018); verification fails closed, so deferral is not a security gap. Configure per ADR-018 whenever a silent-update need appears (keygen + public-key pinning + signed publishing workflow; the code is ready).
