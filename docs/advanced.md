...
- **请求禁用（已实现）**：集合树右键请求可"禁用/启用"（禁用后名称划线置灰）；Runner 跳过禁用请求并计入 skipped，单发不受影响。草稿/排障请求常驻集合但不参与批量运行。
...
- **数据驱动**：每轮把数据文件一行注入 `data` 作用域（优先级见[变量解析](./request-lifecycle.md#2-变量解析与模板引擎)）；脚本内 `pm.info.iteration`/`iterationCount` 提供 1-based 迭代号与总轮数（数据驱动分支脚本的经典诉求，Postman 语义）。
...
- `apirequest-cli list` 列出工作区、集合（含请求计数）与环境（标出激活态），用于发现 `--collection` 与 `--env` 参数。
