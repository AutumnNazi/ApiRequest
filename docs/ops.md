...
| 导入解析器 fuzz | 6 种导入格式（cURL/HAR/Insomnia/OpenAPI/Postman/REST Client）对任意输入不 panic | CI 每 push 60s 探测 + 种子语料回归（`go test -fuzz`；coordinator 收尾偶发超时自动重试一轮，真崩溃不因重试漏过） |
...
| 指标 | 目标 |
|------|------|
| 冷启动到可交互 | < 1.5s（OnDomReady 实测，数值随诊断包导出） |
