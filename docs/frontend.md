...
- **URL ↔ Params 双向同步**：URL 输入框失焦/回车/选中建议时解析 query 段重建 Params 表（base 与 fragment 的编辑不影响表格）；Params 行编辑实时序列化回 URL。`{{var}}` 模板全程原样保留（不做百分号编码）；禁用行从 URL 中剥除（后端 `AddParams` 中 URL query 优先，保留会导致取消勾选不生效）。表格为主但 URL 尚无 query 的存量请求，追加新键而不清表。
- **变量提示**：`{{` 触发变量补全下拉；已定义变量 hover 显示来源与值（密钥掩码）；未定义变量红色下划线。
- **断言助手**（已实现）：脚本面板"测试脚本"页在有响应时提供"插入断言"下拉——状态码 / Content-Type / JSON 顶层字段（上限 5）/ 耗时预算，追加到当前脚本，用户可再调整。
- **响应区**：Pretty/Raw/Preview 切换与搜索高亮；正文最多渲染 500,000 个字符，blob 只允许有界分块查看，完整内容通过原生文件对话框流式另存。HTML blob 不把预览片段当完整文档渲染。
- **响应对比**：响应区「对比」按钮打开对比弹窗——从历史记录（按 URL 预填搜索）中选一条与当前响应做统一行 diff；默认开启「格式归一」（JSON 解析后按排序键 + 两空格缩进重序列化，消除缩进/键序噪声，任一侧非 JSON 则回退原文）；正文超限（40 万字符）或超过 diff 行数上限时提示改用 Body 视图。diff 只做展示，不参与请求发送。
- **错误边界**：根节点包一层全局 `ErrorBoundary`（未捕获渲染错误显示全屏降级页 + 重试/刷新）；每个懒加载浮层（命令面板/Cookie/WS-SSE/设置/gRPC/GraphQL/主题）再各包一层 `DialogErrorBoundary`——单个面板渲染抛错只圈在该浮层内并可关闭，标签页与未保存草稿不受影响，不冒泡到根边界拖垮整个应用。
