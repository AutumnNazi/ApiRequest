...
- ~~字段级同步合并~~ 已实施（[ADR-017](./decisions.md)：三路合并 + 本地基线 + 冲突清单，见 sync.md）。
- ~~应用内更新~~ 全链路已实施（[ADR-018](./decisions.md)：验签 + 版本 floor + 下载校验 + 两阶段替换/回滚 + `cmd/updatetool`）；发布侧密钥无限期搁置（受众为开发者，走 release 页/包管理器更新；验签失败关闭，搁置零安全成本），需要时按 ADR-018 补配。
- ~~桌面端冒烟测试~~ 已扩展：desktop CI 矩阵内含 CLI 核心链路冒烟（平台数据目录、keychain 回退、引擎/Runner）；文件对话框等纯 GUI 交互仍按 ops.md 由交互式冒烟覆盖。
