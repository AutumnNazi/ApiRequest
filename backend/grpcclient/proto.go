...
// （protocompile 编译源文件，import 目录解析依赖）。proto 模式下 Discover 完全
...
// loadProtoFiles 编译 .proto 源文件为描述符集合。includeDirs 为额外 import 根目录；
// proto 文件自身所在目录始终参与解析（go 例行惯例）。google/protobuf 标准类型
// import 无需配置即可解析（WithStandardImports）。
// 返回注册完成的文件集与全部服务全名。
func loadProtoFiles(protoFile string, includeDirs []string) (*protoregistry.Files, []string, error) {
	...
	// SourceResolver 把文件名按 ImportPaths 逐个解析（Windows 绝对路径会被二次拼接），
	// 因此只传文件自身相对其目录的名字——该目录是 ImportPaths[0]
	results, err := compiler.Compile(context.Background(), filepath.Base(protoFile))
	if err != nil {
		return nil, nil, model.NewError(model.KindValidation, "parse proto: "+err.Error())
	}
	files := &protoregistry.Files{}
	// 用户文件 + 全部依赖（含 well-known 标准库文件）都要注册进 registry，
	// 否则 FindDescriptorByName 找不到跨文件类型
	...
	register = func(fd protoreflect.FileDescriptor) error {
		...
		for i := 0; i < imports.Len(); i++ {
			...
			if _, err := files.FindFileByPath(dep.Path()); err != nil {
				// 未注册过的依赖才递归（重复注册报错）
				if rerr := register(dep); rerr != nil {
					return rerr
				}
			}
		}
	...
	for _, f := range results {
		// linker.File 直接实现 google 的 protoreflect.FileDescriptor
